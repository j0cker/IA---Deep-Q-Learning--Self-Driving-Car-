...
# parameter 1 is 5 -  3 sensors, orientation, negative orientation (states - 5 dimensions)
# parameter 2 is 3 actions (left, straight and right - input size)
# parameter 3 is gama = 0.9
brain = Dqn(5,3,0.9)
# is a vector of 3 elements. Action = 0 (index) => no rotation and go straight, action = 1 (index) => rotate 20 degres to the right, action = 2 (index) => rotate -20 degres to the left
action2rotation = [0,20,-20]
# initializing the last reward or current  reward in real time. If the car doesnt go to the sand is positive if get into any sand is negative
last_reward = 0
# is a vectir initializing the mean score curve (sliding window of the rewards) with respect to time (it stores the rewards through time)
scores = [] 
...
def init():
    ...
    global goal_x # x-coordinate of the goal (where the car has to go, that is the airport (upper left of the screen) or the downtown) 
    global goal_y # y-coordinate of the goal (where the car has to go, that is the airport or the downtown)
    global first_update 
    # initializing the sand array with only zeros, longueur is the long of the map and largeur is the large of the map
...
# Initializing the last distance or the current position of the car
last_distance = 0

#the coordinates of the origin is 0 0 which are in the downside left of the map 
...
class Car(Widget):
    ...
    rotation = NumericProperty(0) # initializing the last rotation (current rotation) of the car (after playing the action, the car does a rotation of 0, 20 or -20 degrees)
    ...
    # initializing the signal received by sensor 1 is the density of sand around sensor 1
    signal1 = NumericProperty(0)
    # initializing the signal received by sensor 2 is the density of sand around sensor 2
    signal2 = NumericProperty(0) 
    # initializing the signal received by sensor 3 is the density of sand around sensor 3
    signal3 = NumericProperty(0) 
    # how do we calculate the density of sand? we take some big squares around each of the sensors
    # these are actually (en realidad) squares of 200 by 200 and for each of the squares, we divide
    # the number of ones in the square by the total number of cells in the square that is 20 times
    # 20 equals 400 and that gives us the density of sand because the ones correspond to the sand
    # we do this for each sensor and that gives us the density of sand around each sensor that is
    # the signals
    ...
    def move(self, rotation):
        ...
        # updating the position of sensor 1 Vector(30, 0) is the distance between the car and what the car detects
        ...
        # we get the x coordinate of our sensor, then we take all the cells from minus 10 to plus 10
        # then we do the same for the Y coordinate taking all the cells from minus 10 to plus 10
        # so that we get the square of 20 by 20 pixels surrounding the sensor, and inside this square 
        # we sum all the ones, so basically we sum all the cells (cause all the cells contain either 1 or 0)
        # we divide the number of ones in the square by the total number of cells
        # that is 20 times 20 equals 400 cells and that gives us the density of sand because the ones correspond to 
        # the sand we do this for each sensor and that gives us the density of sand around each sensor that is
        # the signals
        ...
        # penalizamos al coche si los sensores se acercan a los muros
        # so we assing a number 1 which is a total amount of sand which is very bad which is the worst you can get
...
class Game(Widget):
    ...
    def update(self, dt): # the big update function that updates everything that needs to be updated at each discrete time t when reaching a new state (getting new signals from the sensors)
        ...
        # vectors dimensions
        # orientation, -orientation so that our AI learn and explores in both directions (right or left)
        last_signal = [self.car.signal1, self.car.signal2, self.car.signal3, orientation, -orientation]
        # playing the action from our ai (the object brain of the dqn class) this is our neural network
        # it returns the action to play at each time.
...
#paint some roads or some obstacules.
